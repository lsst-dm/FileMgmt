...
def parse_cmd_line(argv):
    ...
    parser.add_argument('--relpath', action='store', required=True, help='relative path on disk within archive (no archive root)')
...
def validate_args(dbh, args):
    ...
    """
    archive_root, archive_path, relpath, ignore, ignore1 = dbutils.get_paths_by_path(dbh, args)
    ...
    if not os.path.exists(path):
        print "Warning: Path does not exist on disk:  %s" % (path)
...
def main():
    ...
    archive_root = validate_args(dbh, args)

    #print archive_root

    files_from_disk = diskutils.get_files_from_disk(args.relpath, archive_root, args.md5sum, args.debug)
    files_from_db = dbutils.get_files_from_db(dbh, args.relpath, args.archive, args.debug)
