...
def parse_cmd_line(argv):
    ...
    parser.add_argument('--archive', action='store', required=False, default='desar2home',help='archive_name from file_archive_info table')
    parser.add_argument('--reqnum', action='store', required=True, help='Request number to search for')
    parser.add_argument('--unitname',action='store', required=True, help='Unit name to search for')
    parser.add_argument('--attnum', action='store', required=True, help='Attempt number to search for')
...
def validate_args(dbh, args):
    ...
    """
    archive_root, archive_path, relpath, ignore, ignore2 = dbutils.get_paths_by_id(dbh, args)
    ...
    return archive_root, archive_path, relpath
...
def diff_files(comparison_info, files_from_db, files_from_disk, check_md5sum, check_filesize):
    ...
    if len(comparison_info['path']) > 0:
        ...
        for fname in sorted(comparison_info['path']):
            fdb = files_from_db[fname]
            fdisk = files_from_disk[fname]
            print "\t%s %s %s" % (fname, fdisk['relpath'], fdb['path'])
...
def main():
    ...
    archive_root, archive_path, relpath = validate_args(dbh, args)

    files_from_db = dbutils.get_files_from_db(dbh, relpath, args.archive)
