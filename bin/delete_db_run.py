...
def delete_using_run_vals(dbh, tablename, reqnum, unitname, attnum, verbose=0):
    ...
    sql = "delete from %s where unitname='%s' and reqnum='%s' and attnum='%s'" % (
        tablename, unitname, reqnum, attnum)
    if verbose >= 2:
        print("\n%s\n" % sql)
    curs = dbh.cursor()
    curs.execute(sql)
    if verbose >= 1:
        print("%3s deleted rows" % curs.rowcount)
...
def delete_db_run(dbh, unitname, reqnum, attnum, verbose=0):
    ...
    sql = "select metadata_table, wgb.filename from wgb, ops_filetype where wgb.unitname='%s' and wgb.reqnum='%s' and wgb.attnum='%s' and wgb.filetype=ops_filetype.filetype" % (
    ...
    sql = "select log from pfw_wrapper where unitname='%s' and reqnum='%s' and attnum='%s'" % (
        unitname, reqnum, attnum)
    if verbose >= 2:
        print(sql)
    curs = dbh.cursor()
    ...
    sql = "select junktar from pfw_job where unitname='%s' and reqnum='%s' and attnum='%s'" % (
        unitname, reqnum, attnum)
    if verbose >= 2:
        print(sql)
    curs = dbh.cursor()
    ...
    sql = "select id from pfw_exec where unitname='%s' and reqnum='%s' and attnum='%s'" % (
        unitname, reqnum, attnum)
    curs.execute(sql)
    ...
    sql = "select id from pfw_wrapper where unitname='%s' and reqnum='%s' and attnum='%s'" % (
        unitname, reqnum, attnum)
    if verbose >= 2:
        print(sql)
    curs.execute(sql)
    ...
    for table, filelist in list(del_by_table.items()):
